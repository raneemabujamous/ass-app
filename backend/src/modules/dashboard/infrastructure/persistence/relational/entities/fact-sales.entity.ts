...
export class FactSalesEntity {
  ...
  @Index() @Column({ type: 'int' }) product_id!: number;

  @Column({ type: 'int' }) quantity!: number;
  @Column({ type: 'numeric', precision: 12, scale: 2 }) unit_price!: string;
  @Column({ type: 'numeric', precision: 12, scale: 2 }) discount!: string;
  @Column({ type: 'numeric', precision: 12, scale: 2 }) line_subtotal!: string;
  @Column({ type: 'numeric', precision: 12, scale: 2 }) tax_amount!: string;
  @Column({ type: 'numeric', precision: 12, scale: 2 }) line_total!: string;
